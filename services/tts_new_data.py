...
# Nghiệp vụ: Hệ thống TTS Mới - Mobile Internet (FastAPI Async/Multi-threaded Core Engine)
...
import ttsnew_api
from ttsnew_api import (
    get_ttsnew_tickets_for_precheck, 
    api_transfer_ttsnew_ticket, 
...
def _process_single_ticket(
    ...
    # 1. Kiểm tra xem thuê bao đã có nhận định / nội dung xử lý hợp lệ trong DB chưa
    ...
    if can_reuse_db:
        state.log("INFO", f"   ↳ 📋 [{ticket_code}] Thuê bao {phone_84} đã có kết quả trong DB: [{existing_db_row['status']}]. Kế thừa hiển thị.")
        ...
        def _fetch_cem():
            ...
            return cem_recs, app_evts, c_str, a_str

        # CHẠY ĐỒNG THỜI 3 DỊCH VỤ CORE
        with ThreadPoolExecutor(max_workers=3) as inner_exec:
            f_bt = inner_exec.submit(_fetch_btools)
            f_sp = inner_exec.submit(_fetch_sapc)
            f_cm = inner_exec.submit(_fetch_cem)

            clean_data = f_bt.result()
            sapc_result, info_result = f_sp.result()
            cem_records, app_events, cem_data_str, app_usage_str = f_cm.result()

        # Lưu thông tin SAPC & Cell Profile
        ...
        # Lưu file JSON BTools vào number/ (hoặc fallback dùng lại dữ liệu chu kỳ trước nếu lần này lỗi)
    ...
    if reopen_count > 0:
        ...
        state.log("WARN", f"⚠️ Phiếu {ticket_code} ({phone_84}) mở lại {reopen_count} lần -> KHÔNG TỰ ĐỘNG ĐÓNG!")
...
def execute_tts_new_data_cycle():
    ...
    try:
        ...
        # Tiền xử lý nạp cookie BTools trước để các luồng con không xung đột truy cập Selenium Driver
        driver_lock = threading.Lock()
        with driver_lock:
            try:
                get_btools_cookie(driver)
            except Exception:
                pass

        # Khởi tạo SAPC & CEM Clients chia sẻ qua kết nối HTTP keep-alive
        try:
        ...
        with ThreadPoolExecutor(max_workers=max_ticket_workers) as outer_exec:
            ...
            for fut in as_completed(future_to_ticket):
                if state.stop_requested:
                    state.log("WARN", "Nhận được yêu cầu dừng tiến trình tiền kiểm TTS Mới.")
                    break
                try:
                    fut.result()
                except Exception as ex_fut:
                    t_info = future_to_ticket.get(fut, {})
                    state.log("WARN", f"Lỗi xử lý luồng phiếu {t_info.get('ticket_code')}: {ex_fut}")

        # Sắp xếp lại excel_summary_list theo đúng thứ tự danh sách phiếu ban đầu
